...
def buscar_repositorios_em_alta(linguagem="python", dias=7, quantidade=5):
    """Busca os repositórios mais populares criados recentemente no GitHub."""
    # O argumento do timedelta é em inglês: days=dias
    data_limite = (datetime.now() - timedelta(days=dias)).strftime("%Y-%m-%d")

    url = f"https://api.github.com/search/repositories?q=created:>{data_limite}+language:{linguagem}&sort=stars&order=desc"
...
if __name__ == "__main__":
    print("🔎 Buscando repositórios em alta no GitHub...\n")
    resultados = buscar_repositorios_em_alta(linguagem="python", quantidade=5)
